 ...
 * @returns {Matrix}
 */
export const compose = (...matrixes) =>
  matrixes.reduceRight(multiply, identity);
 ...
 * @returns {Point}
 */
export const transform = (point, matrix) => {
  const {
    a1, b1, c1, // r1
    a2, b2, c2, // r2
    // 0, 0, 1  // r3
...
const quarturns = [0, 1, 0, -1, 0];

/**
 * @param {number} a
 * @returns {Matrix}
 */
export const rotateQuarturn = a => {
  const sin = a % 4;
  const cos = (a + 1) % 4;
  return {
    a1:  quarturns[cos], b1: -quarturns[sin], c1:  0,
    a2:  quarturns[sin], b2:  quarturns[cos], c2:  0,
    a3:  0,              b3:  0,              c3:  1,
  };
}
 ...
 * @returns {string}
 */
export const matrixStyle = ({a1, b1, c1, a2, b2, c2}) => 
